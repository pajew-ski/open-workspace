...
import { actionEngineTools } from '@/lib/actions/tools';

/**
 * Server-side engine dependencies: assembles the tool belt (workspace
 * actions, API tools, MCP tools), skills and delegable agents for a chat
 * turn executed on the backend.
 *
 * Die Workspace-Fähigkeiten kommen aus der Aktions-Registry
 * (ACTIONS_SPEC §3) und laufen IM PROZESS mit dem Kontext des
 * Chat-Requests — nie über einen HTTP-Aufruf an die eigene Route, der
 * die Identität verlöre.
 */
...
export async function buildServerEngineDeps(actions: ActionContext): Promise<ServerEngineDeps> {
    const [apiTools, skills, agents, mcpTools, actionTools] = await Promise.all([
        loadTools(),
        loadSkills(),
        loadAgents(),
        collectMcpTools(),
        actionEngineTools(actions),
    ]);
