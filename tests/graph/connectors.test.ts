...
describe('Katalog: nur Implementiertes existiert (Invariante 10)', () => {
    it('führt genau die Arten aus M3–M6, a2a-agent-card/mcp-server (M9) und home-assistant', () => {
        expect(listConnectorKinds().map(c => c.kind).sort()).toEqual(['a2a-agent-card', 'git-backup', 'github-rdf', 'home-assistant', 'json-canvas', 'mcp-server', 'obsidian-vault', 'rdf-file']);
