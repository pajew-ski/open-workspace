...
**Vorrang**: Diese Spec hat Vorrang vor der [Analyse (2026-08)](../analyse-2026-08.md) §5 und [TODO.md](../../TODO.md), wo sie ihnen widerspricht. [AGENTS.md](../../AGENTS.md) bleibt für Code-Konventionen, Mobile-First, Safety-Regeln und das E2E-Gate uneingeschränkt gültig.
...
1. **erledigt** — `src/lib/graph/` neu anlegen, ohne Bestehendes anzufassen. Store + Vokabular + Serialisierung + Tests.
2. **erledigt** — **Projektion aus dem Bestand**: ein einmaliger Migrator liest `data/docs/*.md`, `data/tasks/*.json`, `data/canvas/*.json` und erzeugt daraus `data/graph/workspace.nq`. Idempotent, wiederholbar, mit Vorher-Nachher-Bericht. (`bun run migrate:graph`)
3. **erledigt** — `GET /api/graph` behält Route und Antwortformat (schema.org-`@graph`), generiert die Antwort aber per Query aus dem Store. **`color` und `val` fallen aus der Antwort weg**; die Graph-UI bezieht sie aus der Präsentationsschicht bzw. berechnet sie. Das ist ein bewusster Breaking Change am internen Contract.
4. **erledigt** — Schreibpfade umstellen: `src/lib/storage/*` schreibt weiterhin die Dateien (für Git-Lesbarkeit und Obsidian), aber über den Store als Wahrheitsquelle. Doppelte Wahrheit ist ein Übergangszustand mit Ablaufdatum, kein Zielzustand — markiere die Übergangsstellen mit `// MIGRATION:` und liste sie in `TODO.md`. *(Erledigt mit M6-Session: Store-first-CRUD in `src/lib/graph/workspace/`, `src/lib/storage/*` sind Fassaden, alle Marker aufgelöst — Abnahme `tests/graph/workspace-roundtrip.test.ts`.)*
5. **erledigt** — `src/lib/ontology/generator*.ts` wurde zu `src/lib/graph/projection/` und läuft nur noch aus dem Store bzw. als dessen View. Die alten Generatoren sind gelöscht, nicht auskommentiert.
...
**Erfüllt, als Tests verankert (`tests/graph/multi-user.test.ts` — jede Zeile der Matrix ein eigener Negativtest über einen ECHTEN Leak-Pfad: die Kante `alice:notiz —ow:linksTo→ bob:geheim` liegt im ERLAUBTEN Graphen, nur ihr Ziel nicht; dazu `tests/graph/acl.test.ts` für Modell, Standardregeln, Räume, Freigabehandlung und den öffentlichen Teilgraphen).** Umsetzungsnotizen: Die Rechte sind **RDF im selben Store**, nicht Konfiguration — `graph/acl` trägt Web-Access-Control-Regeln (`acl:Authorization`, `acl:accessTo` auf die Graph-IRI, `acl:agent`/`acl:agentGroup`/`acl:agentClass`, `acl:mode`), gelesen und geschrieben ausschließlich von `authz/acl-graph.ts`. Die Modus-Implikation, die WAC offenlässt, ist hier festgelegt und begründet: `control` ⊃ `write` ⊃ `append`/`read`, `append` allein ist die Briefkasten-Semantik aus §17.2. Rollen (`reader`/`contributor`/`editor`/`owner`) sind benannte Modus-Bündel für die UI — die Mitgliederliste eines Raums IST die Menge der Regeln auf seinem Graphen, es gibt keine zweite Liste. **Nichts ist per Default sichtbar, auch nicht für den Eigentümer**: Sein `control` steht als Tripel im Graphen (angelegt beim Start durch `ensureDefaultAuthorizations`, das nur auffüllt und nie überschreibt), nicht als Sonderfall im Code — damit ist Eigentum per SPARQL prüfbar. `graph/acl` selbst trifft kein Muster und keine Regel; wer Regeln sehen will, geht über die Verwaltungs-API, die `control` auf dem betroffenen Graphen verlangt. Der **Dataset-Resolver aus §17.3 ist zwei Stufen**: `grantForIdentity` (Identität + `graph/acl` ⇒ `AccessGrant`) und die schon bestehenden `resolveDataset`/`retrievalDataset`, die den Grant als Verengung nehmen und injizieren — deshalb hat sich die Grant-Schnittstelle aus M10 nicht geändert und MCP-Server wie eingehende Föderation liefen unverändert weiter; getauscht wurde nur die Herkunft (`OW_MCP_TOKENS` nennt jetzt den Nutzer, die Scopes sind eine optionale zusätzliche Verengung). Ein Architekturtest erzwingt §17.3 wörtlich: Jede Datei unter `src/lib/graph` und `src/app/api`, die `store.query(` aufruft, muss ihr Dataset nachweislich vom Resolver beziehen — dafür wurde auch die schema.org-Projektion (`/api/graph`) auf `resolveDataset` umgestellt. SPARQL UPDATE bekam `writableGraphs`: gesetzt heißt, alles außerhalb ist geschützt — auch ein Graph, den es noch nicht gibt (sonst legte ein Update fremde Graphen an); die Ablehnung unterscheidet nicht zwischen „systemverwaltet", „fremd" und „existiert nicht". Der **Anfrage-Kontext** (`server/context.ts`) bindet Identität, nutzerskalierte IRI-Fabrik und Grant zusammen und zieht die Identität über `next/headers` — Server-Komponenten wie Route-Handler erreichen sie darüber, ein durchgereichter Parameter wäre an fünfzig Stellen vergessen worden. Sicherheitskritisch und als Test verankert: Läuft ein Anmeldeverfahren und fehlt die geprüfte Identität, ist die Anfrage **anonym**, nicht der Einzelnutzer — `resolveIdentity` liefert den sonst als Platzhalter zurück, und ungefiltert wäre jeder fehlende Header ein Generalschlüssel. §17.5 ist gebaut: `graph/u/<id>/public` ist über die reguläre Standardregel (`foaf:Agent` `Read`) anonym lesbar und föderierbar, `GET /.well-known/void` beschreibt den Umfang **des Anfragenden** als `void:Dataset`, und Entitäts-IRIs dereferenzieren unter `/u/<userId>/<type>/<id>` mit Content Negotiation (Turtle/JSON-LD/HTML mit eingebettetem JSON-LD) — mit der ehrlichen Grenze, dass das eine HTTP-Instanz-Base voraussetzt und die Default-URN-Base es nicht kann (404 mit Hinweis statt Attrappe); Rate-Limit für anonyme Zugriffe über denselben `RateLimiter` wie M10/M11. Die Nebenkanäle aus §17.4 haben je einen eigenen Test: Such- und Vektorindex sind pro Dataset gebaut (nicht global mit gefilterter Trefferliste), die Retrieval-Klammer greift vor der Expansion, der Reasoner läuft je Nutzer über dessen Graphen, der Bound-Join der ausgehenden Föderation wird über den ausgehenden Query-Text geprüft, gesperrt und nicht existent antworten byte-gleich. **Export und Git-Sync** folgen §17.4 wörtlich: Der Export liefert nur die Verzeichnisse des Nutzers (`data/u/<id>/…`; instanzweite Bestände nur für Verwalter), und ein `git-backup` verlangt `control` auf JEDEM Graphen des Snapshots — sonst 403 mit Begründung, weil ein gemeinsames Repo mit gemischten Graphen genau das ist, was §17.4 ausschließt. `graph/acl` wird als `data/graph/acl.nq` **neben** dem Manifest gesichert: Rechte müssen einen Neustart überleben, dürfen aber über keinen manifest-getriebenen Weg zurückgelesen oder ausgeliefert werden (kein `bidirectional`-Restore, kein Nutzer-Export). Ein Git-Backup, dessen Ziel `data/graph` selbst ist, committet die Datei als Teil des Arbeitsverzeichnisses mit — zulässig, weil ein Backup nach derselben Regel `control` auf jedem enthaltenen Graphen voraussetzt. Ein neuer Term (`ow:Space` samt `ow:spaceGraph` — bewusst nicht `ow:targetGraph`, dessen `rdfs:domain ow:Connector` über OWL RL jeden Raum zum Connector machte, und §4.4 verbietet das Umdefinieren bestehender Terme); alles Übrige ist Standard-Vokabular (WAC, FOAF, VoID). UI: `/graph/access` — Identität, sichtbare Graphen mit Modi, Räume, Freigaben; sichtbar ist dort nur, was der Nutzer wirklich verwaltet. `capabilities.multiUser` steht damit für `server`/`ha-addon` auf true. Betriebsdoku: [docs/multi-user.md](../multi-user.md).

**M14 — Selbstmodell und Einführungsstrecke (§18)** 
Das Modell der Anwendung in `graph/meta` (Module, Seiten, Entitätstypen, verfügbare Connector-Arten, aktive Runtime-Fähigkeiten, Schema-Version), der Systemkontext des Assistenten aus dieser Abfrage statt aus gepflegtem Prompt-Text, und die geführte Einführung, die den Graphen an sich selbst erklärt.
*Abnahme*: „Was kann dieses System" und „welche Module gibt es" sind per SPARQL beantwortbar; ein neu registriertes Modul ist dem Assistenten ohne Prompt-Pflege bekannt; keine Seite der Anwendung fehlt im Selbstmodell und kein Eintrag existiert ohne Seite; die vier Schritte der Einführungsstrecke sind reale Aktionen im echten Graphen, ihr Fortschritt ist RDF, und Zurücknehmen stellt den Vorzustand kanonisch byte-identisch wieder her.
**Erfüllt, als Tests verankert (`tests/graph/self-model.test.ts`, `tests/graph/onboarding.test.ts`).** Umsetzungsnotizen: Die Modul-Registry (`src/lib/app/modules.ts`) ist die EINE Code-Quelle — aus ihr entstehen die Hauptnavigation UND das Selbstmodell; die früher an zwei Stellen gepflegte `MODULE_CONTEXT`-Tabelle im Assistenten und im CopilotKit-Provider ist gelöscht, und ein Test verbietet ihre Wiederkehr ebenso wie eine Registry, die von `src/app/**/page.tsx` abweicht (die „vierte Wahrheit" aus §18). Der Generator (`graph/meta/self-model.ts`) folgt dem Muster des AI-Spiegels aus M9: abschnittsweiser Replace in `graph/meta` über die eigenen Entitäts-Präfixe (`app/`, `module/`), deterministisch, beim Start aufgerufen; ändert sich nichts, wird auch kein Snapshot geschrieben. **Ehrlich statt vollständig**: Ein Modul, dessen Runtime-Fähigkeit fehlt (`/graph/sparql` in `local`), erscheint gar nicht — `ow:capability` trägt nur aktive Fähigkeiten (Invariante 10). Neue Terme (§4.3): `ow:Module`, `ow:route` (Route statt `schema:url`, weil der Base-Path erst zur Installationszeit feststeht — M12), `ow:entityType`, `ow:capability`, `ow:runtime`, `ow:availableConnectorKind` (bewusst nicht `ow:connectorKind`, dessen Domain die Anwendung über OWL RL zum Connector machte — dieselbe Lehre wie `ow:spaceGraph` in M13) und `ow:OnboardingStep`; Schema- und Anwendungsversion nutzen `schema:schemaVersion`/`schema:softwareVersion`. Der Assistent liest das Modell **serverseitig** (`/api/chat` über `readSelfModel`, geklammert vom Grant): Was der Client mitschickt, ist Eingabe, keine Wahrheit über das System; im serverlosen Browser-Betrieb gibt es kein Modell, und dann behauptet der Prompt nichts (statt eine veraltete Liste zu wiederholen). Die **Einführungsstrecke** (`/onboarding`, `graph/onboarding/*`) hat keinen Übungsmodus: Schritt 1 fragt das Selbstmodell wirklich ab, Schritt 2 legt ein Dokument über die Store-first-CRUD an, Schritt 3 importiert prima-materia über den EINEN Connector-Vertrag (github-rdf, commit-gepinnt), Schritt 4 zählt die Aussagen je Herkunft. Der Fortschritt ist eine `ow:OnboardingStep`-Aktivität pro Nutzer in `graph/meta` (`prov:used`/`prov:generated`/`prov:endedAtTime`) — kein UI-Zustand, deshalb geräteübergreifend und per SPARQL prüfbar; „rückgängig" löscht das Erzeugte und die Aufzeichnung, nachgewiesen über den kanonischen Dump (RDFC-1.0) vor der Strecke und danach. Ein fehlgeschlagener Import wird nicht als erledigt verbucht, der Connector bleibt aber für den zweiten Versuch stehen. Beim Live-Lauf fiel dabei eine Lücke aus M13 auf und wurde geschlossen: Der Grant kommt ausschließlich aus `graph/acl`, dessen Standardregeln beim Start entstehen — ein Graph, der ERST ZUR LAUFZEIT angelegt wird (Connector-Import, Inferenz-Materialisierung), hatte keine Regel und war damit bis zum nächsten Neustart unsichtbar; `ensureGraphAuthorizations` zieht sie im Anfrage-Kontext nach (Auffüller wie beim Start, No-Op ohne neue Graphen, Negativ- und Positivfall als Test). Die Herkunfts-Zählung (`graph/provenance.ts`, `GET /api/graph/provenance`) speist auch den neuen Abschnitt „Herkunft" im Graph-Explorer (§11) — nativ, importiert und inferiert werden dort mit denselben Zahlen unterschieden, inferierte Kanten weiterhin gestrichelt und per Default aus.

**M15 — Kalender, Chats und AI-Konfiguration als Graph-Bürger** 
Nachziehen dessen, was nach M14 als einziger Bestand neben dem Graphen lag: Termine und Chat-Verlauf lagen instanzweit in `data/` und erschienen deshalb in keinem Modul als Entitätstyp — das Selbstmodell wartete seit M14 genau darauf.
*Abnahme*: Termine und Unterhaltungen sind per SPARQL beantwortbar (welcher Termin aus welchem Abonnement, welche Nachricht in welcher Rolle), laufen Store-first (ein externer Datei-Edit ändert die Wahrheit nicht), erben Nutzergraphen und ACL ohne neuen Mechanismus, und die neuen Entitätstypen stehen an ihren Modulen; die Präsentations-Trennung (Invariante 2) gilt unverändert.
**Erfüllt, als Tests verankert (`tests/graph/graph-citizens.test.ts`, erweitert in `tests/graph/workspace-roundtrip.test.ts`, `tests/graph/search.test.ts`, `tests/graph/shacl.test.ts`, `tests/graph/agents-skills-tools.test.ts`).** Umsetzungsnotizen: KEINE neue Pipeline — Kalender und Chats gehen denselben Weg wie Dokumente und Aufgaben (§12.4): `WorkspaceSnapshotInput` trägt sie, `migrate/from-files.ts` bildet vorwärts ab, `workspace/read.ts` zurück, `workspace/files.ts` projiziert; `storage/calendar.ts` und `storage/chat.ts` sind nur noch Fassaden. Standardklassen vor eigenen (Invariante 8): ein Abonnement ist ein `schema:DataFeed` mit `schema:url`, seine Termine sind `schema:Event` mit `schema:isPartOf` darauf, ein Dialog ist `schema:Conversation` mit `schema:hasPart` → `schema:Message`; eigene Terme nur für das, was keine Entsprechung hat (`ow:enabled`, `ow:allDay`, `ow:messageRole`). Der ICS-Abruf bleibt, was er ist — ein Netz-Lauf; sein Ergebnis geht in EINER Mutation in den Store, mit derselben replace-Semantik wie ein Connector-Import (§6.2), damit nie ein Zustand entsteht, in dem der Kalender als abgerufen gilt und seine Termine die alten sind. Invariante 2 kostete zwei Präsentations-Terme: die generative A2UI-Oberfläche einer Antwort (`ow:generativeSurface`) und die zuletzt geöffnete Unterhaltung (`ow:selected`) stehen in `graph/<u>/presentation`, ebenso die Kalenderfarbe — die Ersetzung läuft über Subjekt-Präfixe und trägt bewusst KEIN `schema:isPartOf`, weil `pruneOrphanCanvasLayouts` das als Layout-Gruppe einer gelöschten Pinnwand läse. Die AI-Konfiguration bleibt operative Konfiguration (sie muss auch serverlos laufen) und wird wie in M9 GENERIERT gespiegelt: Provider als `ow:InferenceProvider` (`ow:providerKind`, `ow:toolCallMode`, `ow:endpoint`), ihre KONFIGURIERTEN Modelle als `ow:Model` über `schema:provider` — kein Geheimnis, kein erfundenes Live-Inventar (Invariante 10, dasselbe Prinzip wie beim MCP-Tool-Inventar). Zwei Folgen außerhalb des Graph-Kerns: Der Global Finder holt Termine und Chats jetzt aus demselben Volltext-Index wie alles andere — der in M8 ehrlich benannte Sonderweg an den Storages vorbei ist entfallen, womit auch die Zugriffsgrenzen aus §17.4 für sie gelten (ein Treffer im Nachrichtentext führt zur Unterhaltung, `/assistant?id=…`); und der Snapshot steht auf Schema 3, damit ein v2-Manifest die einmalige (Re-)Migration des Dateibestands auslöst, genau wie beim Übergang v1 → v2.
